...
class Dataset(Data.Dataset):
    ...
    def __getitem__(self, index): 
        ...
        # output = []
        # max_length = 40
        # for seq in self.history[index]:
        #     result = np.ones(max_length, dtype=seq.dtype) * 1
        #     if seq.shape[0] > max_length:
        #         result = seq[:max_length]
        #     else:
        #         result[:seq.shape[0]] = seq
        #     output.append(result)
        # pads = 10 - len(output)
        # for i in range(pads):
        #     output.append([1]*max_length)
    ...
    def __init__(self, query, his, fts, cap, trg, trg_y, pad, vids, qa_ids, cuda=False, audio_fts=None):
        ...
        if fts is not None and len(fts)!=0:
            if len(fts[0].shape)==4: # Full scale spatio-temporal features
                ...
                self.spatial_mask = self.to_cuda((self.fts.sum(2).sum(-1)!=0).unsqueeze(-2))
                self.temporal_mask = self.to_cuda((self.fts.sum(1).sum(-1)!=0).unsqueeze(-2))
            else:
                self.fts = self.to_cuda(torch.from_numpy(fts[0]).float())
