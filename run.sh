...
include_caption=summary # concatenate caption and summary together
...
nb_blocks=3             # number of attention blocks
d_model=256            # feature dimensions
...
expid=${fea_names}_warmup${warmup_steps}_epochs${num_epochs}_dropout${dropout}_faster-rcnn_recome_2_new_decoder_seed1_recome_2_hisqcapvideo  # output folder name
...
workdir=`pwd`
labeled_test=''
train_set=$data_root/text_data/train_set4DSTC7-AVSD.json
valid_set=$data_root/text_data/valid_set4DSTC7-AVSD.json
if [ $decode_data = 'off' ]; then
  test_set=$data_root/text_data/test_set4DSTC7-AVSD.json
  labeled_test=$data_root/text_data/lbl_undiscloseonly_test_set4DSTC7-AVSD.json
...
#        git clone git://github.com/tylin/coco-caption utils/coco-caption
...
# if [ $stage -le 3 ]; then
    ...
    echo -----------------------------
    if [ $decode_data = 'off' ]; then
        fea_file="<FeaType>_test/<ImageID>.pkl"
    fi
